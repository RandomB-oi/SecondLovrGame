...
vec4 getShaderColor(vec2 fragCoord) {
	// return fractalRender(vec2(fragCoord.x*1000+250, fragCoord.y*1000));
...
vec4 lovrmain()
{
	...
	vec4 baseColor = Color * textureColor;

	if (textureColor.x >= 1 && textureColor.y >= 1 && textureColor.z-0.4 < 0.05) {
		float x = FragCoord.x/screenHeight;
		float y = FragCoord.y/screenHeight;
		baseColor = getShaderColor(vec2(x, y)*2);
		diffuse = vec4(1,1,1,1);
		// return vec4(abs(sin(FragCoord.x/5+Time)), abs(sin(FragCoord.y/5+Time)), 0, 1);
		// return vec4(noiseVal.x, noiseVal.y, 0, 1);
	}

	// if (fogAlpha == -1) { // skybox
	// 	return baseColor;
	// }

	return lerpColor(baseColor * (ambience + diffuse), fogColor, fogAlpha);
};
